...
class ChickenGame extends FlameGame with HasDraggables {
  ...
  Future<void>? onLoad() async{
    ...
    background = SpriteComponent()..sprite = await loadSprite('background.png')
    ..size = size;
    add(background);

    print("2. load the assets for the game");
    Image  chickenImage = await images.load("chicken.png");
    var chickenAnimation = SpriteAnimation.fromFrameData(chickenImage, SpriteAnimationData.sequenced(amount: 14, stepTime: 0.1, textureSize: Vector2(32,34)));
    chicken = SpriteAnimationComponent()
    ..animation = chickenAnimation
    ..size = Vector2(32, 32) * chickenScaleFactor
